...
Live → arushmangal.github.io/study-tracker
---
Why this exists
...
Calls the YouTube Data API for the live `likeCount` on each tracked comment
Compares against previous counts stored in KV and records any new likes
Returns a JSON payload — `totalLikes`, `totalHours`, a per-comment `breakdown`, and `lastChange` metadata
...
Go to Settings → Pages → Source → Deploy from a branch, select `main` / `/ (root)`, and save. The page will be live at `https://arushmangal.github.io/study-tracker` within a minute.
---
Built by Arush — JEE aspirant, trying to stay off YouTube one like at a time.
